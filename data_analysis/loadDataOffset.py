        ...
        if self.ctn:
            ...
            for i in range(0,n_sensor,1):
                ...
                if json_data:
                    ...
                    r = row_count(filename_csv)
                    if r < offset:
                        offset = 0
                    print("Rows %d offset %d " % (r, offset))
                    del json_data, json_parsed, json_parsed_datapoints
                # Offset
                while offset > 0:
                    url = "https://kw.koruza.net/api/v1/stream/%s/?granularity=%s&limit=%s&offset=%s&value_downsamplers=min&value_downsamplers=mean&value_downsamplers=max&time_downsamplers=mean&start=%s&end=%s&format=json" %(self.sensor_ID[i], granularity, limit,str(offset), start, end)
                    # Save Json to file
                    json_data = urllib.urlopen(url)
                    if json_data:
                        print ("Saved offset %s for unit %s " % (ID_name[i], self.name))
        ...
        if self.ctn:
            ...
            for i in range(0,n_weather,1):
                ...
                if json_data:
                    ...
                    # Check if all data was read
                    offset = offset_global
                    if row_count(filename_csv) < offset:
                        offset = 0
                    del json_data, json_parsed, json_parsed_datapoints
                # Offset
                while offset:
                    ...
                    json_data = urllib.urlopen(url)
                    if json_data:
                        print ("Saved offset %s for weather station" % (weather_name[i]))
                        json_parsed = json.load(json_data)
                        json_parsed_datapoints = json_parsed["datapoints"]
                        ...
                        with open(filename_csv, 'ab') as f:
                            ...
                            dict_writer.writerows(json_parsed_datapoints)
                        # Check if all data was read
                        if row_count(filename_csv) < offset + 10000:
                            offset = 0
                        else:
                            offset = offset + 10000
                        del json_data, json_parsed, json_parsed_datapoints
...
# Function returning number of lines in csv file
def row_count(input):
    with open(input) as f:
        for n_lines, l in enumerate(f):
            pass
    return n_lines
